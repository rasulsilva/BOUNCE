...
void writeCommand(unsigned char c) {
    ...
    GPIOPinWrite(GPIOA0_BASE, 0x40,0x00);// set low for command
    GPIOPinWrite(GPIOA1_BASE, 0x1,0x00);// set low for OC
    MAP_SPICSEnable(GSPI_BASE); // chip select on
    MAP_SPIDataPut(GSPI_BASE,c); // send parameter through SPI
    MAP_SPIDataGet(GSPI_BASE,&ulDummy); // not sure what this does
    MAP_SPICSDisable(GSPI_BASE);// chip select off
   // GPIOPinWrite(GPIOA1_BASE, 0x20,0x00);// set low when done
...
void writeData(unsigned char c) {
    ...
    GPIOPinWrite(GPIOA0_BASE, 0x40,0x40);// set pin high for DATA
    GPIOPinWrite(GPIOA1_BASE, 0x1,0x00);// set low for OC
    MAP_SPICSEnable(GSPI_BASE);//chip select on
    MAP_SPIDataPut(GSPI_BASE,c);//send parameter through SPI
    MAP_SPIDataGet(GSPI_BASE,&ulDummy);// not sure what this does
